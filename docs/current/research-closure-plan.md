...
状态：核心研究闭环首版已实施；剩余发布阻塞项和能力深化转入[发布就绪与能力深化方案](release-readiness-roadmap.md)
...
本方案定义的统一快照、变化检测、市场预期、敏感性矩阵、Watchlist 扫描、去重提醒、财报后复盘和基础多公司比较均已实现，并连接为可重复运行的研究周期：
...
本文保留首版实现契约和验收基线，不再作为当前执行计划。发布阻塞项、真正的同行企业分析和能力深化见[发布就绪与能力深化方案](release-readiness-roadmap.md)。项目继续不增加自动买卖建议、仓位或交易执行。
...
| 优先级 | 工作包 | 交付结果 | 状态 |
| --- | --- | --- | --- |
| P0 | 统一研究快照与运行编排 | 现有能力进入同一次可追溯研究运行 | 已完成 |
| P0 | 跨期变化引擎 | 财务、风险、guidance、措辞变化使用统一契约 | 首版已完成 |
| P1 | 市场预期与估值敏感性 | 实际值、用户预期和估值假设可比较 | 首版已完成 |
| P1 | Watchlist 增量监控 | 一次扫描、去重提醒、失败隔离 | 首版已完成 |
| P1 | 财报后复盘 | Thesis 与实际结果形成长期研究记忆 | 首版已完成 |
| P2 | 多公司比较 | 使用标准化事实横向比较，不做自动荐股 | 基础版已完成 |
| P0 | 验证与候选发布 | 满足 `v0.1.0` 发布门槛 | 转入新方案 |
